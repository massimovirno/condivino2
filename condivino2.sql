...
CREATE TABLE IF NOT EXISTS `oste` (
  ...
  PRIMARY KEY (`num_evento`),
  KEY `username_oste` (`username_oste`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

--
-- RELATIONS FOR TABLE `oste`:
--   `username_oste`
--       `utente` -> `username`
--   `num_evento`
--       `evento` -> `num_evento`
...
CREATE TABLE IF NOT EXISTS `partecipante` (
  ...
  PRIMARY KEY (`username_partecipante`,`num_evento`),
  KEY `num_evento` (`num_evento`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

--
-- RELATIONS FOR TABLE `partecipante`:
--   `username_partecipante`
--       `utente` -> `username`
--   `num_evento`
--       `evento` -> `num_evento`
--
...
CREATE TABLE IF NOT EXISTS `vino` (
  `num_vino` int(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`num_vino`),
  KEY `nome_vino` (`nome_vino`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

--
-- RELATIONS FOR TABLE `vino`:
--   `num_vino`
--       `evento` -> `num_evento`
--
...
('2', 'Monte Sant Urbano', 'Speri Fratelli', 'Amarone della Valpolicella DOCG', 'corvina', 2012, 'Vino dai profumi ampissimi e dalla complessità indiscussa questo Amarone sfida il tempo prestandosi ad un invecchiamento anche lungo in bottiglia', 'img/amarone.jpg'),
('3', 'Tignanello', 'Antinori', 'Toscana IGT', 'sangiovese', 2014, 'Tignanello, in origine "Chianti Classico Riserva vigneto Tignanello" e dal 1971 Toscana IGT con il nome di Tignanello. Dal 1982 la composizione è rimasta la stessa di quella attuale. Tignanello viene prodotto soltanto nelle annate migliori. Il meglio dell''esperienza Antinori in un vino senza eguali che non può mancare nella vostra collezione!', 'img/tignanello.jpg'),
...
CREATE TABLE IF NOT EXISTS `evento` (
  `num_evento` int(11) NOT NULL AUTO_INCREMENT,
...
ALTER TABLE `oste`
  ...
  ADD CONSTRAINT `oste_ibfk_2` FOREIGN KEY (`num_evento`) REFERENCES `evento` (`num_evento`) ON DELETE CASCADE;
...
ALTER TABLE `partecipante`
  ...
  ADD CONSTRAINT `partecipante_ibfk_2` FOREIGN KEY (`num_evento`) REFERENCES `evento` (`num_evento`) ON DELETE CASCADE;

--
-- Limiti per la tabella `vino`
--
ALTER TABLE `vino`
  ADD CONSTRAINT `vino_ibfk_1` FOREIGN KEY (`num_vino`) REFERENCES `evento` (`vino_evento`);
